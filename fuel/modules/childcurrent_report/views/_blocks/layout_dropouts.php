...
$("#txtclickpdf").click(function() {
	...
	if(txtyear=='' || ddlpanchayat == ''  || ddlvillage == ''|| txtreportlevel == ''){   
		...
		success: function(msg){  
			...
			var url = "<?php echo fuel_url('childcurrent_report/childcurrent_dropout_pdf');?>/?"+dataStringone;
			window.open(url);
			}  
		}); 
	}
...
$("#generate").click(function() {
	...
	if(agebreakup=='' || panchayat == ''  || village == ''){   
		...
		dropoutbargraph(); // alert('graph will get generated');
		dropoutpie(); // alert('graph will get generated');
	}
	
});

function dropoutbargraph(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childcurrent_report/dropoutbargraph');?>",  data: "ddlvillage="+village,
	}).done(function(retData) {
		drawdropoutbargraph(retData); 
	});
}
...
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childcurrent_report/dropoutpie');?>",  data: "ddlvillage="+village, 
	}).done(function(yearData) {
		drawdropoutpie(yearData); 
	});
}	

drawdropoutbargraph = function (retData)  {
//alert(retData);
 ...
 $('#container1').highcharts({
            ...
            title: {
                text: 'Dropouts Childrens Population (General Age Break up)'
            },
            
            xAxis: {
                categories: [
                    'Dropouts Childrens Population (General Age Break up)'
                ]
            },
            yAxis: {
                min: 0,
...
drawdropoutpie = function (yearData)  {
