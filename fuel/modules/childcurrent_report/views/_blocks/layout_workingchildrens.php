...
$("#txtclickpdf").click(function() {
	...
	$.ajax({  
		...
		success: function(msg){  
			...
			var url = "<?php echo fuel_url('childcurrent_report/childcurrent_working_pdf');?>/?"+dataStringone;
		    window.open(url);
		}  
	}); 
	}
...
$("#generate").click(function() {
	...
	if(agebreakup=='' || panchayat == ''  || village == ''){   
		...
		workingchildren_bar(); // alert('graph will get generated');
		workingchildren_pie(); // alert('graph will get generated');
	//	workingchildren_piegraph();
...
function workingchildren_bar(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childcurrent_report/workingchildren_bar');?>",  
		data: "ddlvillage="+village,		
	}).done(function(retData) {
		draw_workingchildren_bar(retData); 
	});
}


function draw_workingchildren_bar(retData) 
{
//alert(retData);
 ...
 $('#container1').highcharts({
            ...
            title: {
                text: 'Working Children Population (General Age Break up)'
            },
            
            xAxis: {
                categories: [
                    'Working Children Population (General Age Break up)'
                ]
            },
            yAxis: {
                min: 0,
...
function workingchildren_pie(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childcurrent_report/workingchildren_pie');?>",  
		data: "ddlvillage="+village,	
	}).done(function(getData) {
		draw_workingchildren_pie(getData); 
	});
}	
 


draw_workingchildren_pie = function (getData)  {

//alert(getData);
var d=JSON.parse(getData);
var name = Array();
var data = Array();
