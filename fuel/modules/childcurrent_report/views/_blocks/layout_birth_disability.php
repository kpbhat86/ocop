...
$("#txtclickpdf").click(function() {
	...
	$.ajax({  
		...
		success: function(msg){  
			...
			var url = "<?php echo fuel_url('childcurrent_report/childcurrent_birth_disability_pdf');?>/?"+dataStringone;
		    window.open(url);
		}  
	}); 
	}
...
$("#generate").click(function() {
	...
	if(agebreakup=='' || panchayat == ''  || village == ''){   
		...
		birth_disabilitybargraph(); // alert('graph will get generated');
		birth_disability_pie(); // alert('graph will get generated');
		
	}
	
});

function birth_disabilitybargraph(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childcurrent_report/birth_disabilitybargraph');?>",  
        data: "ddlvillage="+village,	
	}).done(function(retData) {
		drawbirth_disabilitybargraph(retData); 
	});
}

function birth_disability_pie(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childcurrent_report/birth_disability_pie');?>",  
        data: "ddlvillage="+village,	
	}).done(function(yearData) {
		drawbirth_disability_pie(yearData); 
	});
}	

drawbirth_disability_pie = function (yearData)  {
//alert(yearData);
...
for(j=0;j<name.length;j++) { 
   ...
   dataArrayFinal[j] = temp;     
    ...
    $('#container').highcharts({
        ...
        title: {
            text: 'Birth Disability - Wise Childrens Population (General Age Break up)'
        },
        tooltip: {
...
drawbirth_disabilitybargraph = function (retData)  {
...
//alert(retData);
 ...
 $('#container1').highcharts({
            ...
            title: {
                text: 'Birth Disability Childrens Population (General Age Break up)'
            },
            
            xAxis: {
                categories: [
                    'Birth Disability Childrens Population (General Age Break up)'
                ]
            },
            yAxis: {
                min: 0,
