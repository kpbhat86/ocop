<?php include_once(CHILDCURRENT_REPORT_PATH.'views/_blocks/report_toolbar.php'); ?>
...
$("#txtclickpdf").click(function() {
	...
	$.ajax({  
		...
		success: function(msg){  
			...
			var url = "<?php echo fuel_url('childcurrent_report/childcurrent_current_disability_pdf');?>/?"+dataStringone;
		    window.open(url);
		}  
	}); 
	}
...
$("#generate").click(function() {
	...
	if(agebreakup=='' || panchayat == ''  || village == ''){   
		...
		current_disabilitybargraph(); // alert('graph will get generated');
		current_disability_pie(); // alert('graph will get generated');
		
	}
	
});

function current_disabilitybargraph(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childcurrent_report/current_disabilitybargraph');?>",  
        data: "ddlvillage="+village,	
	}).done(function(retData) {
		drawcurrent_disabilitybargraph(retData); 
	});
}

function current_disability_pie(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childcurrent_report/current_disability_pie');?>",  
        data: "ddlvillage="+village,	
	}).done(function(yearData) {
		drawcurrent_disability_pie(yearData); 
	});
}	

drawcurrent_disability_pie = function (yearData)  {
//alert(yearData);
...
for(j=0;j<name.length;j++) { 
   ...
   dataArrayFinal[j] = temp;     
    ...
    $('#container').highcharts({
        ...
        title: {
            text: 'Current Disability Population (General Age Break up)'
        },
        tooltip: {
...
drawcurrent_disabilitybargraph = function (retData)  
{
//alert(retData);
 ...
 $('#container1').highcharts({
            ...
            xAxis: {
                categories: [
                    'Current Disability Population (General Age Break up)'
                ]
            },
            yAxis: {
                min: 0,
