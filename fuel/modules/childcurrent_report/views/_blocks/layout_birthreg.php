...
$("#txtclickpdf").click(function() {
	...
	$.ajax({  
		...
		success: function(msg){  
			...
			var url = "<?php echo fuel_url('childcurrent_report/childbirth_birthregreport_pdf');?>/?"+dataStringone;
		    window.open(url);
		}  
	}); 
	}
...
$("#generate").click(function() {
	...
	if(agebreakup=='' || panchayat == ''  || village == ''){   
		...
		birthregister_bargraph(); // alert('graph will get generated');
		birthregister_pie(); // alert('graph will get generated');
	}
	
});

function birthregister_bargraph(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childcurrent_report/birthregister_bargraph');?>",  data: "ddlvillage="+village,		
	}).done(function(retData) {
		drawbirthregister_bargraph(retData); 
	});
}

function birthregister_pie(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childcurrent_report/birthregister_pie');?>",  
...
drawbirthregister_bargraph = function (retData) {
//alert(retData);
 ...
 $('#container1').highcharts({
            ...
            title: {
                text: 'Birth Registration Childrens Population (General Age Break up)'
            },
            
            xAxis: {
                categories: [
                    'Birth Registration Childrens Population (General Age Break up)'
                ]
            },
            yAxis: {
                min: 0,
...
for(j=0;j<name.length;j++) { 
   ...
   dataArrayFinal[j] = temp;     
    ...
    $('#container').highcharts({
        ...
        title: {
            text: 'Birth Registration - Wise Childrens Population (General Age Break up)'
        },
        tooltip: {
