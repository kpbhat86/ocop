...
$("#txtclickpdf").click(function() {
	...
	$.ajax({  
		...
		success: function(msg){  
			...
			var url = "<?php echo fuel_url('childcurrent_report/childpreprimary_pdf');?>/?"+dataStringone;
		    window.open(url);
		}  
	}); 
	}
...
$("#generate").click(function() {
	...
	if(agebreakup=='' || panchayat == ''  || village == ''){   
		...
		preprimarybargraph(); // alert('graph will get generated');
		preprimarypie(); // alert('graph will get generated');
	}
	
});

function preprimarybargraph(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childcurrent_report/preprimarybargraph');?>",  
        data: "ddlvillage="+village,	
	}).done(function(retData) {
		drawpreprimarybargraph(retData); 
	});
}

function preprimarypie(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childcurrent_report/preprimarypie');?>",  
        data: "ddlvillage="+village,	
	}).done(function(yearData) {
		drawpreprimarypie(yearData); 
	});
}	

drawpreprimarypie = function (yearData)  {
...
drawpreprimarybargraph = function (retData) {
//alert(retData);
