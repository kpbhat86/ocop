...
$("#txtclickpdf").click(function() {
	...
	$.ajax({  
		...
		success: function(msg){  
			...
			var url = "<?php echo fuel_url('childcurrent_report/childnonenrolment_pdf');?>/?"+dataStringone;
		    window.open(url);
		}  
	}); 
	}
...
$("#generate").click(function() {
	...
	if(agebreakup=='' || panchayat == ''  || village == ''){   
		...
		non_enrollbargraph(); // alert('graph will get generated');
		non_enrollreport_pie(); // alert('graph will get generated');
...
function non_enrollbargraph(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childcurrent_report/non_enrollbargraph');?>",  
        data: "ddlvillage="+village,	
	}).done(function(retData) {
		non_enroll_bargraph(retData); 
	});
}
...
function non_enrollreport_pie(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childcurrent_report/non_enrollreport_pie');?>",  
        data: "ddlvillage="+village,	
	}).done(function(yearData) {
		non_enroll_pie(yearData); 
	});
}	


non_enroll_bargraph = function (retData)  
	{
//alert(retData);
 ...
 $('#container1').highcharts({
            ...
            title: {
                text: 'Non Enrolment Population (General Age Break up)'
            },
            
            xAxis: {
                categories: [
                    'Non Enrolment Population (General Age Break up)'
                ]
            },
            yAxis: {
                min: 0,
...
non_enroll_pie = function (yearData)  {
...
for(j=0;j<name.length;j++) { 
   ...
   dataArrayFinal[j] = temp;     
    ...
    $('#container').highcharts({
        ...
        title: {
            text: 'Non Enrolment - Childrens Population (General Age Break up)'
        },
        tooltip: {
