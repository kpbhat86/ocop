...
$("#txtclickpdf").click(function() {
	...
	if(txtyear=='' || ddlpanchayat == ''  || ddlvillage == ''|| txtreportlevel == ''){   
		...
		success: function(msg){  
			...
			var url = "<?php echo fuel_url('childgeneral_report/childgeneral_report_pdf');?>/?"+dataStringone;
			window.open(url);
			}  
		}); 
	}
...
$("#txtclickpdf1").click(function() {
	...
	if(txtyear=='' || ddlpanchayat == ''  || ddlvillage == ''|| txtreportlevel == ''){   
		...
		success: function(msg){  
			var dataStringone =  'ddlpanchayat='+ddlpanchayat+'&ddlvillage='+ddlvillage+'&txtyear='+txtyear+'&txtreportlevel='+txtreportlevel+'&txttaluk='+txttaluk+'&txtdistrict='+txtdistrict+'&txtdivision='+txtdivision+'&txtstate='+txtstate;
			var url = "<?php echo fuel_url('childgeneral_report/childgeneral_Individualreport_pdf');?>/?"+dataStringone;
			window.open(url);
			}  
		}); 
	}
...
function yearSpendTrend(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childgeneral_report/yearSpendTrend');?>",
		data: "ddlvillage="+village,
	}).done(function(retData) {
		drawyearSpendTrend(retData); 
	});
}


drawyearSpendTrend = function (retData)  {
 ...
 $('#container1').highcharts({
            ...
            title: {
                text: 'Age-Wise Childrens Population (General Age Break up)'
            },
            
            xAxis: {
                categories: [
                    'Age-Wise Childrens Population (General Age Break up)'
                ]
            },
            yAxis: {
                min: 0,
