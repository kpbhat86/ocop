...
$("#txtclickpdf").click(function() {
	...
	if(txtyear=='' || ddlpanchayat == ''  || ddlvillage == ''|| txtreportlevel == ''){   
		...
		success: function(msg){  
			...
			var url = "<?php echo fuel_url('childgeneral_report/childgeneral_castewisereport_pdf');?>/?"+dataStringone;
			window.open(url);
			}  
		}); 
	}
...
$("#generate").click(function() {
	...
	if(agebreakup=='' || panchayat == ''  || village == ''){   
		...
		castewisereportbarchart(); // alert('graph will get generated');
		castewisereport_pie(); // alert('graph will get generated');
	
	}
	
});

function castewisereportbarchart(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childgeneral_report/castewisereportbarchart');?>",  data: "ddlvillage="+village,		
	}).done(function(retData) {
		drawCastewisereport(retData); 
	});
}

function castewisereport_pie(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childgeneral_report/castewisereport_pie');?>",  
...
drawCastewisereport = function (retData) {
 ...
 $('#container1').highcharts({
            ...
            title: {
                text: 'Caste-Wise Childrens Population (General Age Break up)'
            },
            
            xAxis: {
                categories: [
                    'Caste-Wise Childrens Population (General Age Break up)'
                ]
            },
            yAxis: {
                min: 0,
