...
$("#txtclickpdf").click(function() {
	...
	$.ajax({  
		...
		success: function(msg){  
			...
			var url = "<?php echo fuel_url('childgeneral_report/religionmultiplemodel');?>/?"+dataStringone;
		    window.open(url);
		}  
	}); 
	}
...
$("#generate").click(function() {
	...
	var txtpanchayat = $('#txtpanchayat').val();
		...
		religionwisereport_multiple_bar(); // alert('graph will get generated');
		religionwisereport_multiple_pie(); // alert('graph will get generated');
	

	
});

function religionwisereport_multiple_bar(){ 
var ddlpanchayat = $("#ddlpanchayat").val();
	document.getElementById('txtpanchayat').value = ddlpanchayat;
	var txtpanchayat = $('#txtpanchayat').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childgeneral_report/religionwisereport_multiple_bar');?>",  data: "txtpanchayat="+txtpanchayat,		
	}).done(function(retData) {
		drawReligionwisereport(retData); 
	});
}

function religionwisereport_multiple_pie(){ 
var ddlpanchayat = $("#ddlpanchayat").val();
document.getElementById('txtpanchayat').value = ddlpanchayat;
	var txtpanchayat = $('#txtpanchayat').val();
	//alert(txtpanchayat);
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childgeneral_report/religionwisereport_multiple_pie');?>",  
		data: "txtpanchayat="+txtpanchayat,	
	}).done(function(yearData) {
		drawReligionwisereport_pie(yearData); 
	});
}	

drawReligionwisereport = function (retData)  {
//alert(retData);
 ...
 $('#container1').highcharts({
            ...
            title: {
                text: 'Religionwise Report Based on multiple Panchayat'
            },
            
            xAxis: {
                categories: [
                    'Religionwise Report Based on mulltiple Panchayat'
                ]
            },
            yAxis: {
                min: 0,
...
drawReligionwisereport_pie = function (yearData)  {
...
for(j=0;j<name.length;j++) { 
   ...
   dataArrayFinal[j] = temp;     
    ...
    $('#container').highcharts({
        ...
        title: {
            text: 'Religionwise Report Based on mulltiple Panchayat'
        },
        tooltip: {
