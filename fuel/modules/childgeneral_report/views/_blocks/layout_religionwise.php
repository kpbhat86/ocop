...
$("#txtclickpdf").click(function() {
	...
	$.ajax({  
		...
		success: function(msg){  
			...
			var url = "<?php echo fuel_url('childgeneral_report/religionreportgenerate');?>/?"+dataStringone;
		    window.open(url);
		}  
	}); 
	}
...
$("#generate").click(function() {
	...
	if(agebreakup=='' || panchayat == ''  || village == ''){   
		...
		religionbargraph(); // alert('graph will get generated');
		religionreport_pie(); // alert('graph will get generated');
		
	}
	
});

function religionbargraph(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childgeneral_report/religionbargraph');?>",  
        data: "ddlvillage="+village,	
	}).done(function(retData) {
		drawreligionbargraph(retData); 
	});
}

function religionreport_pie(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childgeneral_report/religionreport_pie');?>",  
        data: "ddlvillage="+village,	
...
for(j=0;j<name.length;j++) { 
   ...
   dataArrayFinal[j] = temp;     
    ...
    $('#container').highcharts({
        ...
        title: {
            text: 'Religion-Wise Childrens Population (General Age Break up)'
        },
        tooltip: {
...
drawreligionbargraph = function (retData) {
//alert(retData);
 ...
 $('#container1').highcharts({
            ...
            xAxis: {
                categories: [
                    'Religion-Wise Childrens Population (General Age Break up)'
                ]
            },
            yAxis: {
                min: 0,
