...
$("#txtclickpdf").click(function() {
	...
	if(txtyear=='' || ddlpanchayat == ''  || ddlvillage == ''|| txtreportlevel == ''){   
		...
		success: function(msg){  
			...
			var url = "<?php echo fuel_url('childgeneral_report/orphanreportgenerate');?>/?"+dataStringone;
			window.open(url);
			}  
		}); 
	}
...
$("#generate").click(function() {
	...
	if(agebreakup=='' || panchayat == ''  || village == ''){   
		...
		orphanwisereportbarchart(); // alert('graph will get generated');
		orphanwisereport_pie(); // alert('graph will get generated');
		}
	
});

function orphanwisereportbarchart(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childgeneral_report/orphanwisereportbarchart');?>",
		data: "ddlvillage="+village,
	}).done(function(retData) {
		draworphanwisereportbarchart(retData); 
	});
}

function orphanwisereport_pie(){ 
var village = $('#ddlvillage').val();
	$.ajax({
	  	type: "POST",
		url	: "<?php echo fuel_url('childgeneral_report/orphanwisereport_pie');?>", 
		data: "ddlvillage="+village,		
...
draworphanwisereportbarchart = function (retData)  {
//alert(retData);
 ...
 $('#container1').highcharts({
            ...
            title: {
                text: 'Orphan-Wise Childrens Population (General Age Break up)'
            },
            
            xAxis: {
                categories: [
                    'Orphan-Wise Childrens Population (General Age Break up)'
                ]
            },
            yAxis: {
                min: 0,
